...
export default function RejectedIncidents() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/incidents/rejected-by-department/${encodeURIComponent(department)}`
        );
  ...
  const openIncident = (row) => {
    ...
    localStorage.removeItem("view_only"); // rejected incidents should be editable
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      const hay = `${r.incident_id || ""} ${r.title || ""} ${r.created_by_department || ""} ${r.assigned_to_department || ""} rejected`.toLowerCase();
